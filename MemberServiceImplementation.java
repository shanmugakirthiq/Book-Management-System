...
public class MemberServiceImplementation implements MemberService {
    ...
    public void registerMember(MemberDTO memberDTO) {
        // Map MemberDTO to Member entity
        Member member = modelMapper.map(memberDTO, Member.class);
        // If the DTO contains a subscriptionPlan, create a SubscriptionPlan entity.
        if (memberDTO.getSubscriptionPlan() != null) {
            SubscriptionPlan subscriptionPlan = new SubscriptionPlan();
            subscriptionPlan.setSubscriptionId(memberDTO.getSubscriptionPlan().getSubscriptionId());
            // Set additional properties if needed.
            member.setSubscriptionPlan(subscriptionPlan);
        }
    ...
    public void updateMember(Long memberId, MemberDTO memberDTO) {
        ...
        if (memberDTO.getSubscriptionPlan() != null) {
            SubscriptionPlan subscriptionPlan = new SubscriptionPlan();
            subscriptionPlan.setSubscriptionId(memberDTO.getSubscriptionPlan().getSubscriptionId());
            updatedMember.setSubscriptionPlan(subscriptionPlan);
        }
